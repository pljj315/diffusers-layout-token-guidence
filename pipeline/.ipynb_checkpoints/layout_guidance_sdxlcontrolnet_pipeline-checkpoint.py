...
class AttentionStore:
    ...
    def __call__(self, attn, is_cross: bool, place_in_unet: str):
        if self.cur_att_layer >= 0 and is_cross:
            # if attn.shape[1] == np.prod(self.attn_res): # 只存储设定大小的map！！# 4096 = 64**2 / 1024=32**2 / 4096 = 64**2 
            ...
            for attn_map_integrated in self.attention_store.maps(location):
                ...
                for obj_idx in range(object_number):
                    ...
                    if len(token_indices[obj_idx]) != 1:
                        print(len(token_indices[obj_idx]))
                ...
                if i < max_guidance_step and max_guidance_iter_per_step > 0:
                    with torch.enable_grad():
                        ...
                        for guidance_iter in range(max_guidance_iter_per_step):
                            if loss.item()  < 0.2 * scale_factor:
                                print(f'=========break for :{loss.item()}  < 0.2 * {scale_factor}')
                                break
                            ...
                            loss_token = (self._compute_loss_token(max_attention_per_index=max_attention_per_index)* scale_factor)
                            # ===
                            
                            # loss = loss_token
                            loss = ( self._compute_loss(token_indices, bboxes, device)* scale_factor) + loss_token
                            
                            # update latent according to the loss:
                            grad_cond = torch.autograd.grad( loss.requires_grad_(True), [latents], retain_graph=True, )[0]        
                            latents = (latents - grad_cond * self.scheduler.sigmas[i] ** 2)        
                            # print(f"Step:{i+1} Iteration {guidance_iter+1}  | Loss: {loss:0.4f}")
                        print(f"=====step:{i+1} Finished with loss of: {loss:0.4f};scale_factor:{scale_factor}")
